...
# memory = ConversationBufferMemory(memory_key="frontend_buffer", return_messages=True)

tools = [
    tools_writer + tools_explorer + tools_reader
]

prompt_frontend_developer = ChatPromptTemplate.from_messages(
    [
        ("system","""You are an experienced frontend developer.
                    Your goal is to write frontend code to create a user interface for a web application.
...
frontend_developer = AgentExecutor(
        agent=create_tool_calling_agent(
        llm=llm,
        tools=tools,
        prompt=prompt_frontend_developer,
    ),
    tools=tools,
    # memory=memory,
    verbose=True
...
def ask_frontend_developer(input: str) -> str:
    response = frontend_developer.invoke({"input": input})
